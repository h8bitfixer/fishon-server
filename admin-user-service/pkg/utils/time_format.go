...
func TimeStringFormatToTime(timeFormat string, timeSrc string) (time.Time, error) {
	tm, err := time.Parse(timeFormat, timeSrc)
	return tm, err
}

func GetTimeStringFromTime(time2 time.Time) string {
